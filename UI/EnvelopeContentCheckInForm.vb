...
Namespace UI
    ...
    Public Class EnvelopeContentCheckInForm
        ...
        Private m_isUnregisteredRet As Boolean
        Private NewRetailer As Boolean
        ...
        Private Property IsUnregisteredRet() As Boolean
            Get
                Return m_isUnregisteredRet
            End Get
            Set(ByVal value As Boolean)
                m_isUnregisteredRet = value
            End Set
        End Property
...
#End If
        ...
        Protected Overrides Sub EnableDisableControls(ByVal formStatus As FormStateEnum)
            ...
            Select Case formStatus
                Case FormStateEnum.Insert
                    ...
                    Me.PageCountTextBox.Enabled = True
                    ...
                    'Non Required Change
                    If nonRequiredRadioButton.Checked = False Then
                    ...
                    Me.requiredRadioButton.Enabled = True

                    Me.nonRequiredRadioButton.Enabled = True
        ...
        Private Sub SetColumnValues(ByVal tempRow As EnvelopeContentDataSet.vwCircularRow, ByVal dupCheckResponse As DuplicateCheckUserResponse)
            ...
            tempRow.BeginEdit()
            If retailerComboBox.SelectedValue Is Nothing And String.IsNullOrEmpty(retailerComboBox.Text) = False Then
                IsUnregisteredRet = True
                Processor.isNewRetailer = True
                Processor.newRetailerName = retailerComboBox.Text
            End If
            ...
            If isChecked = True Then
                tempRow.CheckInPageCount = CType(PageCountTextBox.Text, Integer)
            End If
            ...
            ElseIf Me.IsUnregisteredRet Then
                Dim userResponse As DialogResult
                userResponse = MessageBox.Show("New Retailer Information was found. Add Retailer to be reviewed ?" _
                                                  , ProductName, MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                If userResponse = Windows.Forms.DialogResult.No Then Exit Sub
                tempRow.StatusID = Processor.GetStatusIdForUnregisteredRetailer
            Else
        ...
        Private Sub ShowRecord(ByVal tempEnvelopeRow As EnvelopeContentDataSet.vwCircularRow)
            ...
            'If Processor.IsVehicleNotRequired(tempEnvelopeRow.VehicleId) = True Then
            '    nonRequiredRadioButton.Checked = True
            'ElseIf Processor.IsVehicleBypassSender(tempEnvelopeRow.VehicleId) Then
            '    nonRequiredRadioButton.Checked = True
            'ElseIf Processor.IsVehicleUnregisteredRetMkt(tempEnvelopeRow.VehicleId) Then
            '    nonRequiredRadioButton.Checked = True
            'ElseIf Processor.IsVehicleUnregisteredRetailer(tempEnvelopeRow.VehicleId) Then
            '    nonRequiredRadioButton.Checked = True
            'Else
            '    nonRequiredRadioButton.Checked = False
            'End If
            ...
            senderName = Nothing
            SenderTextBox.Text = Processor.SenderComment
            ...
            If tempEnvelopeRow.IsDistDtNull() Then
                DistDateTypeInDatePicker.Clear()
            Else
                DistDateTypeInDatePicker.Text = tempEnvelopeRow.DistDt.ToString("MM/dd/yy")
            End If
            ...
            If tempEnvelopeRow.RetId = 0 Then
                Processor.LoadTempRetailer(tempEnvelopeRow.VehicleId)
                retailerComboBox.SelectedValue = 0
            Else
        ...
        Private Function CheckVehicleId(ByVal _envelopeid As Integer) As Boolean
            ...
            Return ReturnVal
        End Function
        Private Function CheckVehicleIdValue(ByVal _envelopeid As Integer) As String

            Dim ReturnVal As String

            ReturnVal = GetFieldValue("select Vehicleid from Vehicle where  EnvelopeId=" + _envelopeid.ToString, "vehicleid")
...
#Region " IForm Implementation "
        ...
        Public Sub Init(ByVal formStatus As FormStateEnum) Implements IForm.Init
            ...
            ' Me.requiredRadioButton.Checked = True
...
#End Region
        ...
        Private Function AreInputsValid() As Boolean
            ...
            If Date.TryParse(DistDateTypeInDatePicker.Text, tempDate) = False Then
                SetErrorProvider(DistDateTypeInDatePicker, "Invalid Date.")
                areAllValid = False
            Else
                RemoveErrorProvider(DistDateTypeInDatePicker)
            End If

            If retailerComboBox.SelectedValue Is Nothing And String.IsNullOrEmpty(retailerComboBox.Text) Then
            ...
            If PageCountTextBox.Text.Trim().Length = 0 And isChecked = True Then
                Me.SetErrorProvider(PageCountTextBox, "Provide Pagecount.")
                areAllValid = False
            Else
                RemoveErrorProvider(PageCountTextBox)
            End If
        ...
        Private Sub adDateTypeInDatePicker_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles adDateTypeInDatePicker.KeyDown
            ...
            'End If
        End Sub
        Private Sub adDateMaskedTextBox_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles adDateTypeInDatePicker.GotFocus
            'Set mask in test control
            'adDateTypeInDatePicker.Mask = adDateMaskedTextBox.Text
        ...
        End Sub
        Private Sub DistDateMaskedTextBox_Validating _
            (ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) _
            Handles DistDateTypeInDatePicker.Validating
            Dim differenceInDays As Integer
            Dim userResponse As DialogResult
            Dim tempDate As DateTime

            RemoveErrorProvider(DistDateTypeInDatePicker)
            If DistDateTypeInDatePicker.Value.HasValue Then
                tempDate = DistDateTypeInDatePicker.Value.Value
                differenceInDays = CType(tempDate.Subtract(System.DateTime.Today).TotalDays, Integer)

                If differenceInDays < -365 Or differenceInDays > 365 Then
                    userResponse = MessageBox.Show("Is the Dist date correct?", ProductName, MessageBoxButtons.YesNo _
                                                   , MessageBoxIcon.Question, MessageBoxDefaultButton.Button2)
                    If userResponse = Windows.Forms.DialogResult.No Then e.Cancel = True
                ElseIf differenceInDays < -28 Or differenceInDays > 28 Then
        ...
        Private Sub loadButton_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles loadButton.Click
            ...
            Try
                ...
                If nonRequiredRadioButton.Checked = True Then
                    Processor.EnableDisableFilter(True, envelopeId)
                End If
                mediaComboBox.DataSource = Processor.Data.Media
                mediaComboBox.ValueMember = "MediaId"
                mediaComboBox.DisplayMember = "Descrip"
        ...
        Private Sub mediaComboBox_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles mediaComboBox.SelectedValueChanged
            ...
            byPass = nonRequiredRadioButton.Checked
            ...
            Else
                ...
                retailerComboBox_SelectedValueChanged(Nothing, Nothing)
        ...
        Private Sub marketComboBox_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles marketComboBox.SelectedValueChanged
            ...
            If Processor.Data.Publication.Count = 1 And marketComboBox.SelectedIndex <> -1 Then
...
#End Region
        ...
        Private Sub retailerComboBox_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles retailerComboBox.KeyDown, retailerComboBox.KeyDown
            'If e.Modifiers = Keys.Control And e.KeyCode = Keys.S And ByPassCheckBox.Checked = True Then
            '    checkInNewButton.Enabled = True
            '    checkInSameButton.Enabled = True
            '    checkInNewAndIndexButton.Enabled = True
        ...
        Private Sub retailerComboBox_Validating _
            ...
            marketid = CType("0" + marketComboBox.SelectedValue.ToString(), Integer)
            mediaid = CType("0" + mediaComboBox.SelectedValue.ToString(), Integer)

            ' isRequired = Processor.IsRetailerExpectedRetailer(retailerId)
            isRequired = Processor.isRequiredRetailers(_SenderID, mediaid, marketid, retailerId)
        ...
        Private Sub checkInButton_Click _
            ...
            If Not retailerComboBox.SelectedValue Is Nothing Then
                retailerId = CType("0" + retailerComboBox.SelectedValue.ToString(), Integer)
                marketid = CType("0" + marketComboBox.SelectedValue.ToString(), Integer)
                mediaid = CType("0" + mediaComboBox.SelectedValue.ToString(), Integer)
            ...
            If dupcheckResponse <> DuplicateCheckUserResponse.Duplicate Then
                If isRequired = True And nonRequiredRadioButton.Checked = True Then
                    Dim userResponse As DialogResult
                    userResponse = MessageBox.Show("This is Required, Push to index. " _
                                                      , ProductName, MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                    If userResponse = Windows.Forms.DialogResult.No Then Exit Sub
                End If
            End If
...
#Region " Processor Events "
        ...
        Private Sub m_envelopeContentProcessor_EnvelopeSenderLoaded(ByVal sender As Object, ByVal e As Processors.EventArgs) Handles m_envelopeContentProcessor.EnvelopeSenderLoaded
            ...
            End If
            If envelopeRow.IsCommentsNull() Then
                SenderComments = String.Empty
            Else
                SenderComments = envelopeRow.Comments
            End If
            ...
            senderName = Nothing
            SenderTextBox.Text = SenderComments
            SenderComments = Nothing
            Me.StatusMessage = String.Empty
        ...
        Private Sub m_envelopeContentProcessor_VehicleLoaded(ByVal sender As Object, ByVal e As Processors.EventArgs) Handles m_envelopeContentProcessor.VehicleLoaded
            ...
            isRequiredRetailer = Processor.isRequiredRetailers(_SenderID, vehicleRow.MediaId, vehicleRow.MktId, vehicleRow.RetId)
        ...
        Private Sub m_envelopeContentProcessor_Synchronized(ByVal sender As Object, ByVal e As Processors.EventArgs) Handles m_envelopeContentProcessor.Synchronized
            ...
            'vehicleId = CheckVehicleIdValue(EnvelopeId)

            If IsUnregisteredRet = True Then
                Processor.AddTempRetailer(tempRow.VehicleId, retailerComboBox.Text)
            End If

            UpdateVehicleIdForDupCheckFormLogId(Integer.Parse(vehicleId), Me.DupcheckFormLogId)
            If IsUnregisteredRet = False Then
                ...
                Else
                    ...
                    isRequiredRetailer = Processor.isRequiredRetailers(_SenderID, mediaid, marketid, retailerId)
                End If
            Else

                isRequiredRetailer = True
                tempRow.SetCheckInOccurrencesNull()
                tempRow.SetColumnError("CheckInOccurrences", "")
                tempRow.StatusID = Processor.GetStatusIdForNewRetailer
            End If

            If Me.PrintBarcode AndAlso isRequiredRetailer Then PrintBarcodeLabel(tempRow)

            Me.PrintBarcode = False
            Me.IsUnregisteredRet = False
...
#End Region
        ...
        Private Sub EnvelopeContentCheckInForm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            'If Processor.IsValidBypassSenderExpectationFilterUser() = 1 Then
            '    ByPassCheckBox.Visible = True

            'Else
            '    ByPassCheckBox.Visible = False
            'End If


            'If Processor.IsValidCheckInRequired = 1 Then
            '    requiredRadioButton.Visible = True
            'Else
            '    requiredRadioButton.Visible = False
            'End If
            'If Processor.IsValidCheckInNonRequired = 1 Then
            '    nonRequiredRadioButton.Visible = True
            'Else
            '    nonRequiredRadioButton.Visible = False
            'End If
        ...
        Private Sub adDateTypeInDatePicker_PreviewKeyDown(sender As Object, e As PreviewKeyDownEventArgs) Handles adDateTypeInDatePicker.PreviewKeyDown
            If e.KeyData = Keys.Tab Then
                adDateTypeInDatePicker.SelectAll()
            End If
        End Sub

        Private Sub requiredRadioButton_CheckedChanged(sender As Object, e As EventArgs) Handles requiredRadioButton.CheckedChanged


            If requiredRadioButton.Checked = True Then
                isChecked = False
                NewRetailer = True

                checkInNewAndIndexButton.Enabled = True
                checkInSameAndIndexButton.Enabled = True
                requiredRetailersLabel.Visible = True
                requiredRetailersDataGridView.Visible = True
                PageCountLabel.Visible = False
                PageCountTextBox.Visible = False
                pgCountLabel.Visible = False
            Else
                checkInNewAndIndexButton.Enabled = False
                checkInSameAndIndexButton.Enabled = False
                requiredRetailersLabel.Visible = False
                requiredRetailersDataGridView.Visible = False
                PageCountLabel.Visible = True
                PageCountTextBox.Visible = True
                pgCountLabel.Visible = True
        ...
        Private Sub nonRequiredRadioButton_CheckedChanged(sender As Object, e As EventArgs) Handles nonRequiredRadioButton.CheckedChanged
        ...
        Private Sub retailerComboBox_KeyPress(sender As Object, e As KeyPressEventArgs) Handles retailerComboBox.KeyPress

            If e.KeyChar = Chr(27) Then 'this is to prevent error when the focus
                e.Handled = True        'is in the combobox and the user presses
            End If                      'escape key

            AutoComplete(retailerComboBox, e, NewRetailer)
        End Sub

        Private Sub PageCountTextBox_TextChanged(sender As Object, e As EventArgs) Handles PageCountTextBox.TextChanged
            Dim digitsOnly As Regex = New Regex("[^\d]")
            PageCountTextBox.Text = digitsOnly.Replace(PageCountTextBox.Text, "")
        End Sub
    End Class
